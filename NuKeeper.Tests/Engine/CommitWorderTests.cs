...
{
    [TestFixture]
    public class CommitWorderTests
    {
        [Test]
        public void MarkPullRequestTitle_UpdateIsCorrect()
        {
            var updates = PackageUpdates.For(MakePackageForV110())
                .InList();
            var sut = MakeDefaultCommitWorder();
        ...
        {
            var updateSetEnricher = new PackageUpdateSetEnricher();
            return new CommitWorder(
                new CommitUpdateMessageTemplate(),
                new DefaultPullRequestTitleTemplate(),
                new DefaultPullRequestBodyTemplate(),
                updateSetEnricher,
                new PackageUpdateSetsEnricher(updateSetEnricher)
            );
        }
